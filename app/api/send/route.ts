...
export async function POST(req: Request) {
  try {
    ...
    const data = await resend.emails.send({
      ...
      subject: `Nueva Solicitud Calificada: ${nombre}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
          <h2 style="color: #d97706; margin-bottom: 5px;">Fénix Construcciones — Solicitud Filtrada</h2>
          ...
          <p><strong>1. N° Predio Municipal / RUC:</strong> ${predio}</p>
          <p><strong>2. Estado documental:</strong> ${etapa}</p>
          <p><strong>3. Rango de Presupuesto:</strong> ${presupuesto}</p>
          <p><strong>Detalle o requerimiento:</strong> ${detalles || 'No especificado'}</p>
        </div>
      `,
    });
